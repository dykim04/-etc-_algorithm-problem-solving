...
void sorted_list_merge(t_list **begin_list1, t_list *begin_list2, int (*cmp)())
{
	cmp("a", "b");
	while (begin_list2)
	{
		sorted_list_insert(begin_list1, begin_list2->data, &strcmp);
...
int main(void)
{
	...
	*(buff1 + 0) = create_string("this");
	*(buff1 + 1) = create_string("is");
	*(buff1 + 2) = create_string("the");
    *(buff1 + 3) = create_string("second");
	*(buff1 + 4) = create_string("buffer2");
	*(buff1 + 5) = NULL;
	...
	*(buff2 + 0) = create_string("this3");
	*(buff2 + 1) = create_string("is3");
	*(buff2 + 2) = create_string("the3");
    *(buff2 + 3) = create_string("third3");
	*(buff2 + 4) = create_string("buffer3");
	*(buff2 + 5) = NULL;

	struct s_list *start2 = NULL;
	create(&start2, buff2, &find_size);
	sorted_list_merge(&start, start2, &strcmp);
	display(start);
